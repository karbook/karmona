...
export default function ShowcaseGrid() {
  ...
  const [shuffledImages, setShuffledImages] = useState<GridImage[]>([]);
  const [currentTextIndex, setCurrentTextIndex] = useState(0);

  const texts = [
    t("Engines and Suspension"),
    t("Restoration and Painting")
  ];

  const shuffleArray = (array: GridImage[]) => {
    return [...array].sort(() => Math.random() - 0.5);
  };

  useEffect(() => {
    setShuffledImages(shuffleArray(gridImages));
